...
```{r}
#mydata <- getGEO(filename="/Users/alba/Desktop/THIRD/OMICS/alejandro/ALBA/OMICS_lab0/GSE147964_series_matrix.txt")
mydata <- getGEO("GSE147964")
```
...
```{r}
min(exprs(smallEset_group1))
mean(exprs(smallEset_group1))
max(exprs(smallEset_group1))
```

GROUP 2: subjects with M. tuberculosis
```{r}
min(exprs(smallEset_group2))
mean(exprs(smallEset_group2))
max(exprs(smallEset_group2))
```

GROUP 3: subjects with M. africanum
```{r}
min(exprs(smallEset_group3))
mean(exprs(smallEset_group3))
max(exprs(smallEset_group3))
```
...
Organe: healthy subjects (group1)
Purple: subjects with M. tuberculosis (group2)
Pink: subjects with M. africanum (group3)
...
TOP 1 GENE:
```{r, echo=FALSE}
barplot(expression_values["NA.1908",], col=colors, las=2, main="gene NA.1908")
```
...
TOP 3 GENE:
```{r, echo=FALSE}
barplot(expression_values["NA.1224",], col=colors, las=2, main="gene NA.1224")
```
